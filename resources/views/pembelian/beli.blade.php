                    ...
                    <div class="card-body pt-0">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5"
                            ...
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-start min-w-100px">Harga Bahan</th>
                            ...
                            @foreach($bahan as $row)
                                <tr>
                                    ...
                                    <td class="test-start pe-0">
                                        <span class="fw-bold ms-3">Rp{{ ($row->harga_satuan_bahan) }},-</span>
                                    </td>
                                    <td class="text-end">
                                        <div class="row">
                                            <div class="form-group w-100px">
                                                <input id="jmlh-{{ $row->id }}" class="form-control" type="number" step="1" min="1" value="0"/>
                                            </div>
                                            <div class="col-md-3 my-auto text-center align-middle w-50px">
                                                <a class="btn btn-sm" id="btn-plus-{{ $row->id }}" row-id="{{ $row->id }}">
                                        ...
                                        @push('bottom')
                                            <script>
                                                $("#btn-plus-{{ $row->id }}").click(function(e) {
                                                    e.preventDefault();
                                                    var rowId = $(this).attr("row-id");
                                                    var jmlh = $("#jmlh-" + rowId).val();
                                                    if(jmlh == 0)
                                                        return

                                                    $.ajax({
                                                        url: "{{ route('pembelian.tambah_keranjang') }}",
                                                        type: "POST",
                                                        data: {
                                                            bahan_id: rowId,
                                                            jumlah: jmlh,
                                                        ...
                                                        success: function(response) {
                                                            Swal.fire({
                                                                text: "Bahan berhasil ditambahkan ke keranjang",
                                                                icon: "success",
